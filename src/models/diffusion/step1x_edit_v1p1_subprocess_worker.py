#!/usr/bin/env python
"""
Step1X-Edit-Preview (v1p1) Subprocess Worker
在特定虚拟环境中运行 Step1X-Edit-Preview 模型推理

用法:
    python step1x_edit_v1p1_subprocess_worker.py --input input.json --output output.json --model-name ... --device cuda:0
"""
...
def main():
    parser = argparse.ArgumentParser(description='Step1X-Edit-Preview Subprocess Worker')
    ...
    try:
        ...
        # 加载模型 (v1p1 使用 Step1XEditPipeline)
        print(f"[Worker] Loading Step1X-Edit-Preview pipeline...", file=sys.stderr)
